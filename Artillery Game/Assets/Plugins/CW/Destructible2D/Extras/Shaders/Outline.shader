		...
		Pass
		{
			CGPROGRAM
				...
				void Frag(v2f i, out float4 o:COLOR0)
				{
					...
					o.a = saturate(_AlphaThreshold + (alphaTex.a - (1.0f - _AlphaThreshold)) * _D2dSharpness);
					
					// Apply outline with custom threshold
					float outline = saturate(_OutlineThreshold + (alphaTex.a - (1.0f - _OutlineThreshold)) * _D2dSharpness);
